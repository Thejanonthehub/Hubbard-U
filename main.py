...
class Fe_for_pred:
    ...
    def extract_features(self):
        ...
        try:
            ...
            avg_nn_dist = np.mean([
                np.mean([
                    np.linalg.norm(n['site'].coords - structure[i].coords)
        ...
        return np.array([
            ...
            atomic_props["electron_affinity"]
        ]).reshape(1, -1)
...
# -----------------------------
# Entry point (this runs when you call `upredictor`)
# -----------------------------
...
def main_entry():
    ...
    # --- Display logo ---
    logo_text = pyfiglet.figlet_format("UPredictor", font="slant")
    colored_logo = colored(logo_text, "cyan")
    print(colored_logo)

    print("Welcome to the UPredictor ML Engine!")
    print("------------------------------------")
    print()
